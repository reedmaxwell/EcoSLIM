...
* **ASCII ET Output Frequency (etwrite):** Controls ASCII ET output (`runname_ET_summary.filenum.out.txt`). Refer to the **Outputs** section for the details on this output. If frequency is set to 0 this output will not be written at all, 1 will write the output every timestep, n>1 will write the output every n timesteps (i.e. n=2 writes outputs every other step)
* **VTK Bianary Grid File Output Frequency (icwrite):** Controls VTK, binary grid based output, Refer to the **Outputs** section for the details on this output. If frequency is set to 0 this output will not be written at all, 1 will write the output every timestep, n>1 will write the output every n timesteps (i.e. n=2 writes outputs every other step) **##Replace with file names**
...
occur.
2. `runname_particle_restart.bin`: Binary file containing all the particle information for the last time step of the simulation. Ten columns: X, Y, Z, Age, Saturated Age, Mass, Source, Status, Concentration, Exit ExitStatus
3. `runname_exited_particles.bin`
  * Time
  * X
  * Y
  * Z
...
Transient files outputs are written at the frequencies specied in the slimin.txt file
1. `runname_transient_particle.filenum.3D` - ipwrite 1242,
3. `runname_ET_summary.filenum.out.txt`: Gridded summary of particles leaving by ET. Columns are: i, j, k, ET_Particles, ET_Mass, ET_Age, ET_Source, Saturation, Porosity
...
+ 2 Y = Y coordinate [L]
+ 3 Z = Z coordinate [L]
+ 4 Age = Particle Residence Time [T] **#**
+ 5 Saturaged Age= Saturated particle residence time [T] **#Expand**
+ 6 Mass= Particle Mass [M]
+ 7 Source = Particle source type (1= particles initialized in the domain, 2=particles added by a rain event, 3= particles added by a snow event) [-]
+ 8 Particle Status = Flag indicating whether particle is currently active (1=active, 0=inactive) [-]
+ 9 Concentration= []**#Expand**
+ 10 Exit Status = Flag indicating how inactive particles left the domain (1=outflow, 2=ET) [-]

**Gridded Variables**
Gridded values are aggregated to the *ParFlow* grid cells at every *ParFlow* timestep
+ 1 Concentration = Active particle Mass/ Mass of water in cell, (Total Particle Mass)/(dx*dy*dz*porosity*saturation) [-]
+ 2 Mean Age = Mass weighted average age of active particles in a cell [T]
+ 3 Mass = Active particle mass in a cell [M]
+ 4 Mean Source = Mass weighted average source of active particles in a cell [-]
+ 5 Concentration ? = Mass weighted particle concentration **#CHECK**
+ 6 ET_Particles = Number of particles leaving as ET from a cell [-]
+ 7 ET_Mass = Total mass of particles leaving as ET from a cell [M]
+ 8 ET_Age  = Mass weighted average age of particle leaving as ET from a cell [T]
+ 9 ET_Source = Mass weighted average source of particles leaving as ET from a cell [-]
