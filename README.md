...
+ **Particle ASCII:** Detailed output of every particle in ascii FORMAT written to a single file.
  ...
  * Variables: *X, Y, Z, Age*
  * Note: This format is very slow in parallel and is NOT advised for large particle numbers. This file is only written if *ipwrite* is less than zero in the `slimin.txt`
...
+ 5 *Saturated_Age*= Saturated particle residence time [T] **#Expand**
...
+ 8 *Status* = Flag indicating whether particle is currently active (1=active, 0=inactive) [-]
+ 9 *Particle_Concentration*= Placeholder, currently set to 1 (**#**)
+ 10 *Exit_Status* = Flag indicating how inactive particles left the domain (1=outflow, 2=ET) [-]
...
+ *1 Grid_Particle_Concentration* = Total active particle mass in cell divided by the mass of water in cell [-]
+ *2 Mean_Age* = Mass weighted average age of active particles in a cell [T]
+ *3 Grid_Mass* = Active particle mass in a cell [M]
+ *4 Mean_Source* = Mass weighted average source of active particles in a cell [-]
+ *5 Grid_Concentration* = Mass weighted *Particle_Concentration*
