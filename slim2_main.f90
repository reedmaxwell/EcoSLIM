...
!!!!  !!$OMP PARALLEL DO PRIVATE(ii)
        do ii = 1, np_active
        call moveparticle(((ithread)*iparticle_group+1),((ithread+1)*iparticle_group))  !, &
        call moveparticle(1, np_active)  !, &

!        np, nx, ny, nz, P,Vx, Vy, Vz, Saturation, Porosity  &
!        ,dx, dy, dz, EvapTrans,pfdt, denh20, Xmax, Ymax, Zmax,  Xmin, Ymin, Zmin, dtfrac)
...
!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!
!!!  Move Particle Subroutine
!!!!!!!!!!!!!!!!!!


subroutine moveparticle(i1, i2) !, np, nx, ny, nz, P,Vx, Vy, Vz, Saturation, Porosity, &
!dx, dy, dz, EvapTrans, pfdt, denh20, Xmax, Ymax, Zmax, Xmin, Ymin, Zmin, dtfrac)
use slim

implicit none
!! global variables passed in
integer i1, i2  ! from-to particle number


!integer, intent(in):: np
!real*8  P(np,10)  !! particle array
!real*8, intent(in):: Saturation(nx, ny, nz)
!!real*8, intent(in):: Porosity(nx, ny, nz)
!real*8, intent(in):: EvapTrans(nx, ny, nz)
!real*8, intent(in):: Vx(nx+1, ny, nz)
!real*8, intent(in):: Vy(nx, ny+1, nz)
!real*8, intent(in):: Vz(nx, ny, nz+1)
!real*8, intent(in):: dx, dy, dz(nz)
!real*8, intent(in):: Xmax, Ymax, Zmax
!real*8, intent(in):: Xmin, Ymin, Zmin
!real*8, intent(in):: dtfrac
!real*8, intent(in):: pfdt
!integer, intent(in):: nx, ny, nz
!real*8, intent(in):: denh20
...
! in the 3D file
write(12,61) float(ii), P(ii,1), P(ii,2), P(ii,3), P(ii,4)
61  FORMAT(5(e12.5))
flush(12)
exit
else
...
! Write out the particle's new location X, Y, Z and its time
! in the 3D file
write(12,62) float(ii), P(ii,1), P(ii,2), P(ii,3), P(ii,4)
62  FORMAT(5(e12.5))
flush(12)
end if

write(14,61) float(ii), P(ii,1), P(ii,2), P(ii,3), P(ii,4)
